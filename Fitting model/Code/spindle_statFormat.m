...
savefile = 'all_affs_stat_array_no_buffer_All';
...
stat_array = {'affnum','model','pert_type','trial','R_squared',...
    'kF','kdF','o_F','o_dF','ka','kv','kl','o_a','o_v','o_l','bgF','bgIFR','maxF','maxIFR',...
    'maxL','maxV','maxA','maxdF','AdjR_squared','AIC','BIC','AICc','J','DI_1s', 'DI_500ms'};

for affnum = 1:length(affs);
    affname = ['aff' num2str(affs(affnum))];
    pathname = cd(cd(['..' filesep 'Data' filesep 'proc_data_no_buffer07-Aug-2017' filesep affname '_proc']));
    ...
    for trial = 1:length(directory) % For all files in directory
        ...
        if strfind(filename,'proc.mat')    % If current file is .mat file
            ...
            for model = 1:7% For each model
                for pert = 1:size(fit_data(model).opt_gains,1)
                    
                    R_squared = fit_data(model).R_squared(pert);
                    ...
                    if model == 1;
                        k_F = fit_data(model).opt_gains(pert,1);
                        k_dF = fit_data(model).opt_gains(pert,2);
                        o_F = fit_data(model).opt_gains(pert,3);
                        o_dF = fit_data(model).opt_gains(pert,4);
                        k_a = 0;
                        k_v = 0;
                        k_l = 0;
                        o_a = 0;
                    ...
                    elseif model == 2;
                        ...
                        o_F = 0;
                        o_dF = 0;
                        k_a = fit_data(model).opt_gains(pert,3);
                        k_v = fit_data(model).opt_gains(pert,2);
                        k_l = fit_data(model).opt_gains(pert,1);
                        o_a = fit_data(model).opt_gains(pert,6);
                        o_v = fit_data(model).opt_gains(pert,5);
                        o_l = fit_data(model).opt_gains(pert,4);
                    elseif model == 3;
                        k_F = 0;
                        k_dF = 0;
                        o_F = 0;
                        ...
                        k_l = fit_data(model).opt_gains(pert,1);
                        o_a = 0;
                        o_v = fit_data(model).opt_gains(pert,4);
                        o_l = fit_data(model).opt_gains(pert,3);
                    elseif model == 4;
                        k_F = 0;
                        k_dF = 0;
                        o_F = 0;
                    ...
                    elseif model == 6;
                        k_F = fit_data(model).opt_gains(pert,1);
                        k_dF = fit_data(model).opt_gains(pert,2);
                        o_F = fit_data(model).opt_gains(pert,6);
                        o_dF = fit_data(model).opt_gains(pert,7);
                        k_a = fit_data(model).opt_gains(pert,5);
                        k_v = fit_data(model).opt_gains(pert,4);
                        k_l = fit_data(model).opt_gains(pert,3);
                        o_a = fit_data(model).opt_gains(pert,10);
                        o_v = fit_data(model).opt_gains(pert,9);
                        o_l = fit_data(model).opt_gains(pert,8);
                    elseif model == 7;
                        k_F = 0;
                        k_dF = 0;
                        o_F = 0;
                        o_dF = 0;
                        k_a = 0;
                        k_v = fit_data(model).opt_gains(pert,1);
                        k_l = fit_data(model).opt_gains(pert,3);
                        o_a = 0;
                        o_v = fit_data(model).opt_gains(pert,2);
                        o_l = fit_data(model).opt_gains(pert,4);
                    end
                   
                    % Adjusted R squared & Information Criterion
                    J = fit_data(model).terminal_cost(pert);
                    logL = log(1/J);
                    numParam = numel(fit_data(model).opt_gains(pert,:)) + 1;  % Estimated Variance necessarily included as a parameter
                    ...
                    stat_array{end,25} = AIC;
                    stat_array{end,26} = BIC;
                    stat_array{end,27} = AICc;
                    stat_array{end,28} = J;
                    stat_array{end,29} = DI1;
                    stat_array{end,30} = DI2;
